...
using Bluestone.Droid.Services.FileViewer;
using Bluestone.Mobile.Presentation.Helpers;
using Bluestone.Mobile.Presentation.Services.FileViewer;
...
namespace Bluestone.Droid.Services.FileViewer
{
    public class FileViewerService : IFileViewerService
    {
        public async Task<bool> View(Stream stream, string filename)
        {
            try
            {
                ...
                {
                    bool result = await PermissionService.RequestStorageAccess();
                    if (!result)
                    {
                        return false;
                    }
